...
export class Path {
  ...
  /** Create 5m straight curve aligned with end of Path (or at center of rink if Path empty).*/
  createNewEndCurve(): Curve {
    ...
    if (this.curves.length == 0) {
      // 5m straight line at center of rink
    ...
    return new Curve(p0, p0.plus(dir.times(handleLength)), p0.plus(dir.times(5 / 2)), p0.plus(dir.times(5))); // 5m straight line
  }
  ...
  /** Create 5m straight curve aligned with start of Path (or at center of rink if Path empty).*/
