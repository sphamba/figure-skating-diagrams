...
export class Sequence {
  ...
  >(partKey: Key, property: Property, time: number): [KeyframeType, KeyframeType, Relative] {
    ...
    keyframes = keyframes.filter(
      (keyframe) => keyframe.data[property as keyof typeof keyframe.data] !== undefined,
    )
  ...
  >(partKey: Key, property: Property, time: number): Interpolable {
    const [keyframeBefore, keyframeAfter, easedTime] = this.getKeyframesAround(
      partKey,
      property,
      time,
    )
...
function getEasedTime<T extends KeyframeType>(
  ...
  if (transitionStart === 'linear' && transitionEnd === 'linear') {
    ...
    throw new Error(
      `Keyframe transition from "${transitionStart}" to "${transitionEnd}" not implemented`,
    )
  }
